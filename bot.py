...
class StaffActionView(discord.ui.View):
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            # Log acceptance message in the accepted channel
            ...
            # Disable only Accept & Deny buttons, keep Call Player
            ...
            # Remove from applied_users since reviewed
            applied_users.discard(self.applicant.id)
    ...
    async def call(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            # Send DM to applicant with permanent invite
            try:
                invite = await interview_channel.create_invite(max_age=0, max_uses=0)
                await self.applicant.send(
                ...
                await interaction.response.send_message(
                    f"✅ {self.applicant.mention} has been called via DM with a clickable invite link.",
                    ephemeral=True
                )
            except discord.Forbidden:
                await interaction.response.send_message(
                    f"⚠️ Cannot DM {self.applicant.mention}. They might have DMs disabled.",
                    ephemeral=True
                )
...
class YBNDZModal(discord.ui.Modal, title='YBN DZ RolePlay Whitelist'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if interaction.user.id in applied_users:
            await interaction.response.send_message(
                "❌ You already submitted an application. Please wait for it to be reviewed.", ephemeral=True
            )
            return

        # Add user to applied_users to prevent double applications
...
async def setup_ybn(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "• **Steam Link:**\n"
            "• **Invited By:**\n\n"
            "  **Once your application is reviewed and accepted, you will receive access to the server.** "
        ),
        color=discord.Color.dark_gray()
    )
...
>>>>>>> f24a3e0fbed349824f670fa909f5cbd0ee8dde58
bot.run(TOKEN)